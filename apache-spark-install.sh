...
########################################
# CONFIGURE ENV (fixed heredoc)
########################################

for c in "${containers[@]}"; do

# Correct way: use single quotes for the remote command and feed the heredoc externally
lxc exec "$c" -- sudo -u hdoop bash -c 'grep -q SPARK_HOME ~/.bashrc || cat >> ~/.bashrc' <<'EOF'
# Spark
export SPARK_HOME=/opt/spark
export PATH=$PATH:$SPARK_HOME/bin
export HADOOP_CONF_DIR=/opt/hadoop/etc/hadoop
...
########################################
# SPARK CONFIG (NAMENODE) - fixed heredoc
########################################
...
########################################
# TEST (fixed using a script file)
########################################

echo "Running Spark test..."

# Write Scala commands to a local temp file
cat > /tmp/spark_test.scala <<'EOF'
sc.parallelize(1 to 10).count()
System.exit(0)
EOF

# Push the script into the name node
lxc file push /tmp/spark_test.scala "$NameNode"/tmp/

# Execute spark-shell with that script
lxc exec "$NameNode" -- sudo -u hdoop bash -c '
export SPARK_HOME=/opt/spark
export HADOOP_HOME=/opt/hadoop
export HADOOP_CONF_DIR=$HADOOP_HOME/etc/hadoop
export PATH=$PATH:$SPARK_HOME/bin:$HADOOP_HOME/bin
spark-shell --master yarn --deploy-mode client -i /tmp/spark_test.scala
'

# Clean up local temp file
rm -f /tmp/spark_test.scala
